# AK135.py
#
# Return a matrix/vectors containing the depth, vp, vs, rho from the AK135
# reference model (Kennett and Engdahl, GJI, 1995).
#
# Ported from AK135.m by James Wookey
#
# Usage:
#     ak135() - returns the complete ak135 model
#                     (columns are depth, vp, vs and rho)
#
#     ak135(z) - returns a section of the model, linearly interpolated
#                  at the depths in vector/scalar z
#
#     ak135(s) - return a section of the model designated by the
#                      string s. Acceptable values are:
#                        'crust' - (0-35km)
#                        'upper-crust' - (0-20km)
#                        'lower-crust' - (20-35km)
#                        'mantle' - (35-2891km)
#                        'upper-mantle' - (35-660km)
#                        'lower-mantle' - (660-2891km)
#                        'core' - (2891-6371km)
#                        'outer-core' (2891-5153km)
#                        'inner-core' - (5153-6371km)
#
#     [dp,vp,vs,rh] = ak135(...) - returns the parameters in separate vectors
#                                  (all 4 are required)
#
#     [dp,vp,vs,vc,rh] = ak135(...) - also return the bulk sound speed
#                                     (vc^2=vp^2-4/3*vs^2)
#
#   Not that at the discontinuities, the second depth has been displaced
#       down by 1m, so the model is not multivalued in depth.
#
# Porting ak135 is a bit of a side-project, does not appear to be needed for what I want to do RN.
...
#     #   choose subset of model
#             if args[0] == 'crust':
#                 ll = [000:003]
#             elif args[0] == 'upper-crust':
#                 ll = [000:001]
#             elif args[0] == 'lower-crust':
#                 ll = [002:003]
#             elif args[0] == 'mantle':
#                 ll = [004:066]
#             elif args[0] == 'upper-mantle':
#                 ll = [004:019]
#             elif args[0] == 'lower-mantle':
#                 ll = [020:066]
#             elif args[0] == 'core':
#                 ll = [067:135]
#             elif args[0] == 'outer-core':
#                 ll = [067:111]
#             elif args[0] =='inner-core':
#                 ll = [112:135]
#             else:
#                 error(['Unknown region specified: {}'.format(args[0])])
#
#             mod = m[ll,:] ;
#         else:
#             zz = args[0] ;
#             mod = np.zeros(length(zz),4) ;
#             mod(:,1) = zz ;
#             mod(:,2) = interp1(m(:,1),m(:,2),zz,'linear',NaN) ;
#             mod(:,3) = interp1(m(:,1),m(:,3),zz,'linear',NaN) ;
#             mod(:,4) = interp1(m(:,1),m(:,4),zz,'linear',NaN) ;
#     else:
#
#        error('ak135.m requires 0 or 1 argument')
#
#
#     switch nargout
#        case 0
#           error('1 or 4 outputs required')
#        case 1
#           varargout(1) = {mod} ;
#        case 4
#           varargout(1) = {mod(:,1)} ;
...
#           varargout(2) = {mod(:,2)} ;
#           varargout(3) = {mod(:,3)} ;
#           varargout(4) = {(mod(:,2).^2-(4/3).*mod(:,3).^2).^0.5} ;
#           varargout(5) = {mod(:,4)} ;
#        otherwise
#           error('1, 4 or 5 outputs required')
#     end
#
#     return
import numpy as np
...
def ak135_original():
    ...
    mod = np.array([[     0.000,     5.8000,     3.4600,     2.7200],
                        ...
                        [5153.501,    11.0427,     3.5043,    12.7037],
