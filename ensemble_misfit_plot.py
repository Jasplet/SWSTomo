...
def plot_2d_sections(models, m, fname=None):
    '''
    ...
    sec1 = make_section(models, m[1], axis='gamma')
    ax1 = fig.add_subplot(1, 3, 1)
    ax1.tricontour(sec1.strength, sec1.alpha, sec1.misfit, [0.7278798995666488])
    ...
    sec2 = make_section(models, m[0], axis='alpha')
    ax2 = fig.add_subplot(1, 3, 2)
    ax2.tricontour(sec2.strength, sec2.gamma, sec2.misfit, [0.7278798995666488])
    ...
    sec3 = make_section(models, m[2], axis='strength')
    ax3 = fig.add_subplot(1, 3, 3)
    ax3.tricontour(sec3.alpha, sec3.gamma, sec3.misfit, [0.7278798995666488])
...
def make_section(models, value, axis):
    '''
    ...
    if axis not in ['alpha', 'gamma', 'strength']:
        ...
        assert np.less_equal(value, 90)
        assert np.greater_equal(value, 0)
    elif axis == 'gamma':
        cols = ['alpha', 'strength', 'misfit']
        assert np.less_equal(value, 180)
        assert np.greater_equal(value, -180)
    elif axis == 'strength':
        cols = ['alpha', 'gamma', 'misfit']
        assert np.less_equal(value, 0.04)
        assert np.greater_equal(value, 0)
        
    slc = models[(np.isclose(models[axis], value, rtol=1.e-2)) & (models['misfit'] < 1)]
    section = slc[cols]
